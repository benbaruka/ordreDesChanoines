...
import { useProfileUser } from "@/controller/query/profile/useProfile";
import SpinnerLoader from "@/global/spinner/SpinnerLoader";

export default function UserInfoCard({ data }: { data: ResponseMe }) {
  const { isOpen, openModal, closeModal } = useModal();
  const { user } = useAuth();
  const { showAlert } = useAlert();
  const token = user?.data?.token?.token;
  const { mutate: _create, isPending } = useProfileUser(token!);
  ...
  const handleSave = async () => {
    try {
      await _create({
        credentials: {
          email: email,
          full_name: full_name || data?.data?.full_name,
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  <div className="col-span-2">
                    ...
                    <Input
                      disabled
                      type="text"
                      defaultValue={data?.data?.role?.description}
                    />
                  </div>
                </div>
              </div>
