...
// import { _getNotification } from "@/controller/query/notification/notification.service";
...
export default function NotificationDropdown() {
  ...
  // const {
  //   data: notif,
  //   isLoading: loaderNotif,
  //   error: errorNotif,
  // } = useQuery({
  //   queryKey: ["notif"],
  //   queryFn: () => _getNotification(token!, {}),
  //   enabled: !!token, // Active la requête uniquement si `token` est défini
  //   placeholderData: (prevData) => prevData,
  // });
  // useEffect(() => {
  //   const hasUnread = notif?.data?.some((notif) => notif.isRead === false);
              ...
              >
                <span className="relative z-1 block h-10 w-full max-w-10 rounded-full">
                  <Image
                    width={40}
                    height={40}
                    src={item.user.avatar || "/images/user/default.jpg"}
                    alt="User"
                    className="w-full overflow-hidden rounded-full"
                  />
                  <span
                    className={`absolute bottom-0 right-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white ${!item?.isRead ? "bg-success-500" : ""} dark:border-gray-900`}
                  ></span>
                </span>
